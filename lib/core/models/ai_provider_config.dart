...
/// single API key.
enum AiProviderType {
  /// Miruns Cloud — the built-in gateway (default, no user config needed).
  mirunsCloud('Miruns Cloud'),
...
class AiProviderConfig {
  ...
  /// Miruns Cloud — the existing default. No key needed from user.
  static const defaultProvider = AiProviderConfig(
    type: AiProviderType.mirunsCloud,
  ...
  static AiProviderConfig preset(AiProviderType type) {
    switch (type) {
      case AiProviderType.mirunsCloud:
  ...
  bool get requiresApiKey => type != AiProviderType.mirunsCloud;

  /// Whether this is the built-in Miruns Cloud gateway.
  bool get isDefault => type == AiProviderType.mirunsCloud;
  ...
  factory AiProviderConfig.fromJson(Map<String, dynamic> json) {
    final typeName = json['type'] as String? ?? 'mirunsCloud';
    final type = AiProviderType.values.firstWhere(
      (t) => t.name == typeName,
      orElse: () => AiProviderType.mirunsCloud,
    );
