...
| 16  | **Active Workout** | `/sport/active`      | Full-screen real-time training: elapsed timer, HR zone ring, live metrics grid (distance, pace, speed, altitude), brain state indicators (EEG), AI coaching insights (every 2 min), voice prompts via TTS. Phase progression: warmup → active → cooldown → finish. Pause/resume controls. **Minimise** to keep recording in the background — a persistent mini banner shows on every tab with elapsed time, HR, and distance until you return or stop. |
