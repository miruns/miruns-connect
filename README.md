...
| **Lab**                  | Session library for recorded EEG data. Record, replay, rename, tag, compare, and export sessions. Session detail with multi-channel waveform replay (pinch-to-zoom, pan), scrubber, window selector, channel-selectable band power analysis, event markers, and artifact marking. Session comparison overlays spectral profiles of two sessions side by side. Swipe-to-delete, search, and empty-state CTA for quick demo start.                                                       |     | **Cloud Sync & Share** | Sessions auto-sync to **miruns-link** (ephemeral backend) on save. Sync status indicator (syncing/synced/failed with retry). One-tap share generates a unique link (`https://share.miruns.com/{code}`) — copies to clipboard and opens native share sheet. Updates (tags, artifacts, notes) re-sync automatically. Anonymous device identity (no real device ID exposed). |     | **Sport & Workouts** | Full workout lifecycle: choose activity → real-time training with live HR/GPS/EEG metrics → voice coaching through earphones → post-workout feedback (fatigue, energy, mood) → AI performance analysis → pre-workout readiness predictions. Supports 8 activity types, 3 user levels (beginner/intermediate/advanced), and HR zone tracking via Tanaka formula. **Background persistence** — workouts keep recording when you navigate away; a music-player-style mini banner on every tab shows elapsed time, HR, and distance so you never lose track. **Dual-engine TTS** — Kokoro neural voice (WAV streaming) with automatic platform TTS fallback and circuit breaker. See **Sport Workout Flow** below. |
...
| Voice coaching   | `flutter_tts` ^4.2.0 + `just_audio`   | TTS service: Kokoro TTS Gateway (streaming WAV) with platform TTS fallback                      |
| Nutrition API    | Open Food Facts API v2                | Product lookup by barcode — no API key needed                                                   |
| Persistence      | `sqflite` ^2.3.3 + `path`             | Local SQLite (entries, captures, settings) — schema v13                                         |
...
| Cloud sync       | `http` + miruns-link API              | Ephemeral session sharing via [miruns-link](https://miruns-link.fly.dev) backend (MongoDB + S3) |
| Sharing          | `share_plus` ^10.1.0                  | Share journal entries, research data exports & session links                                    |
...
# Optional — Kokoro TTS Gateway (falls back to system TTS if omitted)
KOKORO_TTS_URL=https://kokoro-tts-gateway.fly.dev
KOKORO_TTS_API_KEY=your_kokoro_key_here
```
...
│   │   ├── local_db_service  #   SQLite CRUD (schema v13)
...
│   │   ├── tts_service        #   Kokoro TTS Gateway + platform TTS fallback (circuit breaker)
│   │   ├── research_export_service  # Research-grade export (CSV, EDF+, JSON) for EEG/HR/GPS data
│   │   ├── miruns_link_service  # Cloud sync & sharing via miruns-link API
...
│   │   ├── services/         #   WorkoutService, WorkoutAnalyticsService, VoiceCoachService, ActiveWorkoutNotifier
...
| `ActiveWorkoutState` | Immutable snapshot of a running workout: elapsed time, current phase, HR/GPS/EEG readings, AI insights, pause flag. Emitted by `ActiveWorkoutNotifier` and consumed by both the full-screen training UI and the mini banner.       |
...
Sport Home                          ← pick workout type, see AI prediction
  │
  ├─ START ─────────────────────────── Active Workout Screen (cinematic UI)
  │   ├─ Animated background           Zone-reactive gradient blooms, floating particles, breathing glow
  │   ├─ Glass-morphism design         Frosted controls, metric tiles, bottom bar (BackdropFilter)
  │   ├─ Live HR streaming (BLE)       Pulsing BPM readout + zone-reactive ambient (Z1–Z5)
  │   ├─ GPS tracking                  Speed, distance, altitude, pace
  │   ├─ EEG brain state  (*)         Attention, relaxation, cognitive load, fatigue (EEG bands indicator)
  ...
  │   ├─ Phase progression             Animated phase strip — warmup → active → cooldown → finish
  │   ├─ Staggered entrance            Metrics & insights slide in with delay-offset animations
  │   └─ Background persistence        Minimise to banner — workout keeps recording on any tab
  │
...
| 16  | **Active Workout** | `/sport/active`      | Cinematic full-screen training UI with zone-reactive animated background (radial gradient blooms, floating particles, breathing ambient glow). Glass-morphism design throughout — frosted controls, metric tiles, and bottom bar. Pulsing BPM readout scales with heartbeat, phase strip with animated glow on the active segment, staggered entrance animations. Live metrics grid (distance, pace, speed, altitude), EEG bands indicator, AI coaching insights (every 2 min), voice prompts via TTS. Phase progression: warmup → active → cooldown → finish. Pause/resume controls. **Minimise** to keep recording in the background — a persistent mini banner shows on every tab with elapsed time, HR, and distance until you return or stop. |
...
Schema version lives in `local_db_service.dart` (`_schemaVersion`, currently **13**).
...
- [x] Kokoro TTS Gateway integration (`TtsService`) — high-quality neural TTS via REST API with streaming WAV playback (`just_audio`), automatic fallback to platform TTS on failure, circuit breaker (3 failures → 5 min backoff)
...
**Added:** Cloud sync via miruns-link backend — sessions auto-sync on save with status indicator, one-tap share link (`share.miruns.com`), re-sync on edits (tags, artifacts, notes), anonymous device identity, full signal data in sync payload. DB schema v13.
